...
int xPlayerPosition, yPlayerPosition, xTemp, yTemp, xKey, yKey, monsters,
    smartMonsters = 0, userScore;
int xMonster[40], yMonster[40], xSmartMonster[5], ySmartMonster[5];
bool key = 0, keyExist = 1;
string username;
fstream f;
typedef pair<int, string> playerInfo;
...
int main() {
    ...
    cout << "\n----------------------\nEnter your name so that I could laugh "
    ...
    while (true) {
        system("CLS");
        cout << "Press W,A,S,D to move, X to stand still, I to see instruction "
                "and Q to quit game.\n"
             << endl;
        DrawField();
        Choice();
        if (choice == 'I' || choice == 'i')
            continue;
        else if (MoveChecker(xTemp, yTemp)) {
...
void Intro() {
    ...
    Sleep(2000);
    cout << "You know that there is a key in the room and if you can take it, "
            "you can open the exitgate and get out."
         << endl;
    Sleep(2000);
}
...
void Instruction() {
    ...
    cout << "Press W to move up, A to move left, S to move down and D to move "
            "right. You are not allowed to go outside the room. Press X if you "
            "don't want to move."
         << endl;
    cout << "The monsters can move! If they're around you, just stand there "
            "and don't breathe!"
         << endl;
    cout << "Notice that you must find the key BEFORE you enter the exit. Now "
...
int SelectMode() {
    while (true) {
        cout << "Select difficulty: [E]asy, [M]edium and [I]nsane? ";
        cin >> difficulty;
        if (difficulty == 'E' || difficulty == 'M' || difficulty == 'I')
            break;
        else
    ...
    case 'E':
        return 0;
    case 'M':
        return (rand() % 10 + 10); // Medium level has 10 - 20 monsters
    case 'I':
        smartMonsters = 5;
        return (rand() % 15 + 15); // Insane level has 15 - 30 monsters
    }
}
...
bool ChoiceChecker(char choice) {
    if (choice == 'W' || choice == 'A' || choice == 'S' || choice == 'D' ||
        choice == 'I' || choice == 'Q' || choice == 'X' || choice == 'w' || choice == 'a' || choice == 's' || choice == 'd' ||
        choice == 'i' || choice == 'q' || choice == 'x')
        return 1;
    else
        return 0;
}
...
void Choice() {
    ...
    case 'Q': // Forcequit
        ...
        if (choice == 'Y' || choice == 'y') {
            cout << "Even 3-year-old kids can easily solve this. Baizz you "
                    "dumbass loser :D"
                 << endl;
            exit(0);
        }
...
bool LoseConditionChecker(int x, int y) {
    ...
    if (!(keyExist)) {
        cout << "\nA big monster has got the key. You are too slow! Guess he "
                "must be smarter than you: he did the thing you couldn't do."
             << endl;
        return 1;
    }
    return 0;
...
void MovingMonsters() {
    ...
    for (int i = 0; i < movingNumber; ++i) {
        ...
        case 1: // Move up
            if (xMonster[randomMonster] > 0 &&
                field[xMonster[randomMonster] - 1][yMonster[randomMonster]] ==
                    '-') {
            ...
            break;
        case 2: // Move left
            if (yMonster[randomMonster] > 0 &&
                field[xMonster[randomMonster]][yMonster[randomMonster] - 1] ==
                    '-') {
...
void MovingSmartMonsters() {
    ...
    for (int i = 0; i < smartMonsters; ++i) {
        int minDistance =
            min(abs(xKey - xSmartMonster[i]), abs(yKey - ySmartMonster[i]));
        if (minDistance == xKey - xSmartMonster[i])
            movement = 4;
        else if (minDistance == xSmartMonster[i] - xKey)
            movement = 2;
        else if (minDistance == yKey - ySmartMonster[i])
            movement = 1;
        else
            movement = 3;
        switch (movement) {
...
void CreateRollofHonour() {
    f.open("RollofHonour.txt", ios::out);
    for (int i = 0; i < 20; ++i)
        f << "100 -" << endl;
    f.close();
}
...
void ReadRollofHonour() {
    ...
    stringstream ss;
    ifstream f;
    f.open("RollofHonour.txt");
    for (int i = 0; i < 20; ++i) {
        f >> score;
        getline(f, line);
        ss << line;
        ss >> name;
        player.push_back({score, name});
        ss.clear();
    }
    f.close();
}
...
void UpdateRollofHonour() {
    ...
    f.open("RollofHonour.txt");
    for (int i = 0; i < 20; ++i)
        f << player[i].first << ' ' << player[i].second << endl;
    f.close();
}
...
void DisplayRollofHonour(int x) {
    ...
    cin >> choice;
    if (choice == 'N')
        cout << "Nah, but you still have to see this because I want.\n" << endl;
    else if (choice != 'Y')
        cout << "Seems like you don't understand what I said, but okay here it "
                "is: "
             << endl;
    ...
    for (int i = 0; i < 20; ++i)
        cout << setw(3) << player[i].first << setw(20) << player[i].second << endl;

    cout << "Right above is the memberlist of a club named L053rC1ub..."
         << endl;
    ...
    if (x < 20 && x != 0)
        ...
        cout << "So sad you're not allowed to be the new member. But don't "
                "worry, keep tryhard and the Club will accept you one day! "
                "Besides, let's congrat to the Chairman of Losers: "
             << player[0].first << " for being the most failure in the world!!"
             << endl;
}
...
void RollofHonour() {
    ...
    int place;
    for (int i = 0; i < 20; ++i)
        if (player[i].first > userScore) {
            player.insert(player.begin() + i, {userScore, username});
            place = i;
            break;
    }
    player.resize(20);
